 ...
 *******************************************************************/
static void fm_mso_get_cust_profile(
    pcm_context_t		*ctxp,
    poid_t  			*acct_pdp,
        ...
        pin_errbuf_t        *ebufp)
{
	...
	*s_flistpp = 0;
	...
	/****************************************************************
	 *Apply grace period for next three days but add_on grace same day.
	 ****************************************************************/ 
	if (object_type && (strcmp(object_type, "default") == 0))
    	{
		day_end = day_start + (3 * NUMBER_OF_SECONDS_IN_DAY);
	}
	else
	{
...
//	PIN_FLIST_FLD_SET(s_flistp, PIN_FLD_TEMPLATE, template, ebufp);
	...
	/* PIN_FLD_ACCOUNT_OBJ of PROFILE_CUSTOMER_CARE*/
	...
	/* PIN_FLD_NAMEINFO --> PIN_FLD_CITY */
	tmp_flistp = PIN_FLIST_ELEM_ADD(s_flistp, PIN_FLD_ARGS, 13, ebufp);
	elem_flistp = PIN_FLIST_ELEM_ADD(tmp_flistp, PIN_FLD_NAMEINFO, 1, ebufp);
	PIN_FLIST_FLD_SET(elem_flistp, PIN_FLD_CITY, "", ebufp);

	/* PIN_FLD_CITY of MSO_CFG_GRACE_PERIOD_NTF */
	tmp_flistp = PIN_FLIST_ELEM_ADD(s_flistp, PIN_FLD_ARGS, 14, ebufp);
	PIN_FLIST_FLD_SET(tmp_flistp, PIN_FLD_CITY, "", ebufp);

	/* MSO_FLD_PP_TYPE of PROFILE_CUSTOMER_CARE */
        tmp_flistp = PIN_FLIST_ELEM_ADD(s_flistp, PIN_FLD_ARGS, 15, ebufp);
        elem_flistp = PIN_FLIST_SUBSTR_ADD(tmp_flistp, PIN_FLD_CUSTOMER_CARE_INFO, ebufp);
    	PIN_FLIST_FLD_SET(elem_flistp, MSO_FLD_PP_TYPE, &pp_type, ebufp);

	/* MSO_FLD_PP_TYPE of MSO_CFG_GRACE_PERIOD_NTF */
	tmp_flistp = PIN_FLIST_ELEM_ADD(s_flistp, PIN_FLD_ARGS, 16, ebufp);
	PIN_FLIST_FLD_SET(tmp_flistp, MSO_FLD_PP_TYPE, &pp_type, ebufp);

	tmp_flistp = PIN_FLIST_ELEM_ADD(s_flistp, PIN_FLD_ARGS, 17, ebufp);
	PIN_FLIST_FLD_SET(tmp_flistp, PIN_FLD_ACCOUNT_OBJ, NULL, ebufp);

        tmp_flistp = PIN_FLIST_ELEM_ADD(s_flistp, PIN_FLD_ARGS, 18, ebufp);
        elem_flistp = PIN_FLIST_SUBSTR_ADD(tmp_flistp, PIN_FLD_CUSTOMER_CARE_INFO, ebufp);
    	PIN_FLIST_FLD_SET(elem_flistp, MSO_FLD_LCO_OBJ, NULL, ebufp);

	memset(template, '\0', 1024);

	if (object_type && (strcmp(object_type,"default")==0))
	{
		/* GRACE_PERIOD of MSO_CFG_GRACE_PERIOD_NTF */
		tmp_flistp = PIN_FLIST_ELEM_ADD(s_flistp, PIN_FLD_ARGS, 19, ebufp);
		PIN_FLIST_FLD_SET(tmp_flistp, PIN_FLD_VALID_TO_DETAILS, 0, ebufp);
		...
		strcpy(template,"select /*+ parallel(4) */ X from /purchased_product 1, /product 2, /profile/customer_care 3, /account 4, /mso_cfg_grace_period_ntf 5 where 1.F1.type = V1 and 1.F2 = V2 and 1.F3 <> V3 and (1.F4 >= V4 and 1.F5 < V5) and 1.F6 = 2.F7 and 2.F8 like V8 and 1.F9 = 3.F10 and 1.F11 = 4.F12 and (4.F13 = 5.F14 or 3.F18 = 5.F17) and 3.F15 = 5.F16 and 5.F19 > V19 and 1.F20 <> V20 ");
...
//  Retrieve the Customer Type from Profile Object
    fm_mso_get_cust_profile(ctxp, acct_obj, &s_out_flistp, ebufp);

//  Retrieve the Customer City from Account Object
    fm_mso_get_custcity(ctxp, acct_obj, &cust_city, ebufp);
    if (!cust_city)
    {
    ...
    if (s_out_flistp)
    {
	PIN_FLIST_FLD_PUT(s_out_flistp, PIN_FLD_CITY, cust_city, ebufp);
    }

    fm_mso_get_grace_config(ctxp, s_out_flistp, &out_flistp, ebufp);
    ...
    if (out_flistp)
    {
        ...
        if (qty)
        {
            ...
            // Quantity Parameter is passed only for "add_on_grace" option in MTA, hence adding both days for grace period
            grace_days = PIN_FLIST_FLD_GET(out_flistp, PIN_FLD_VALID_TO_DETAILS, 1, ebufp);
            add_on_grace_days = PIN_FLIST_FLD_GET(out_flistp, PIN_FLD_VALIDITY_IN_DAYS, 1, ebufp);
//            if (grace_days && add_on_grace_days)
            if (add_on_grace_days)
            {
 //               total_days = (*grace_days) + (*add_on_grace_days);
//                grace_date = purchase_end_date + ((total_days) * NUMBER_OF_SECONDS_IN_DAY);
                grace_date = purchase_end_date + ((*add_on_grace_days) * NUMBER_OF_SECONDS_IN_DAY);
            }
            else
            {
                // In case of any error in the parameters, the purchase_end_date doesn't change
                grace_date = purchase_end_date;
        ...
        {
            PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_DEBUG, "Execution with Default Grace Period");
            // Quantity parameter will not be passed for the "default" option in MTA, hence only grace_days is applicable
            grace_days = PIN_FLIST_FLD_GET(out_flistp, PIN_FLD_VALID_TO_DETAILS, 1, ebufp);
            if (grace_days)
            {                
                grace_date = purchase_end_date + ((*grace_days) * NUMBER_OF_SECONDS_IN_DAY);
            }
            else
            {
                // In case of any error in the parameters, the purchase_end_date doesn't change
            ...
            PIN_FLIST_FLD_SET(pdt_flistp, PIN_FLD_STATUS_FLAGS, &status_flag, ebufp);
    ...
    {
		PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_ERROR, "/mso_cfg_grace_period_ntf - Configuration Object Not Found...");
    }
...
CLEANUP:
    PIN_FLIST_DESTROY_EX(&s_out_flistp, NULL);
    if (PIN_ERR_IS_ERR(ebufp)) {
        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
    ...
    pin_errbuf_t		*ebufp)
{
    ...
    PIN_FLIST_FLD_SET(tmp_flistp, MSO_FLD_LCO_OBJ, vp, ebufp);
    ...
    if (search_outflistp)
    {
        ...
        if (res_flistp)
	{
        	*out_flistpp = PIN_FLIST_SUBSTR_TAKE(res_flistp, PIN_FLD_CUSTOMER_CARE_INFO, 0, ebufp ); 
        }
        else
	{
	...
	pin_errbuf_t    *ebufp)
{
    ...
    poid_t          *s_pdp = NULL;
    int64           db = 1;
    char            template[1024] = {"select X from /mso_cfg_grace_period_ntf where (F1 = V1 and F2 = V2 and F3 = V3) or F4 = V4 order by account_obj_id0 desc"};
    int32           s_flags = 0;
    ...
    PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_get_grace_config Input Flist", in_flistp);
    ...
    PIN_FLIST_FLD_COPY(in_flistp, PIN_FLD_CITY, args_flistp, PIN_FLD_CITY, ebufp);

    args_flistp = PIN_FLIST_ELEM_ADD(srch_input_flistp, PIN_FLD_ARGS, 2, ebufp);
    PIN_FLIST_FLD_COPY(in_flistp, MSO_FLD_PP_TYPE, args_flistp, MSO_FLD_PP_TYPE, ebufp);

    args_flistp = PIN_FLIST_ELEM_ADD(srch_input_flistp, PIN_FLD_ARGS, 3, ebufp);
    PIN_FLIST_FLD_PUT(args_flistp, PIN_FLD_ACCOUNT_OBJ, PIN_POID_CREATE(db, "/account", 1, ebufp), ebufp);

    args_flistp = PIN_FLIST_ELEM_ADD(srch_input_flistp, PIN_FLD_ARGS, 4, ebufp);
    PIN_FLIST_FLD_COPY(in_flistp, MSO_FLD_LCO_OBJ, args_flistp, PIN_FLD_ACCOUNT_OBJ, ebufp);
    ...
    {
        res_flistp = PIN_FLIST_ELEM_GET(srch_output_flistp, PIN_FLD_RESULTS, 0, 1, ebufp);
	if (res_flistp)
	{
	...
	pin_errbuf_t            *ebufp)
{
	...
	/**********************************************************************************
	 * NT: Comment out parent-child dependency logic
		...
		while ((results_flistp = PIN_FLIST_ELEM_GET_NEXT(child_serv_flist, 
			PIN_FLD_RESULTS, &elem_id1, 1, &cookie1, ebufp)) != NULL)
		{
