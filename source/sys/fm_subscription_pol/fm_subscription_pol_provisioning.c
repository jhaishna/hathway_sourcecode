...
#ifndef lint
static	char	Sccs_id[] = "@(#)$Id: fm_subscription_pol_provisioning.c /cgbubrm_7.5.0.portalbase/1 2015/11/27 06:17:29 nishahan Exp $";
#endif
...
#define CATVSERVICE         "/service/catv"
#define CATVCONFIG	        "/mso_cfg_catv_pt"
#define BBSERVICE         "/service/telco/broadband"
#define BBCONFIG	        "/mso_config_bb_pt"
...
static void plp_ip(pcm_context_t *ctxp, poid_t *svc_obj_p, int32 buy,
	char *tag, pin_errbuf_t *ebp);
static void plp_catv(pcm_context_t *ctxp, poid_t *svc_obj_p, int32 buy,
	char *tag, pin_errbuf_t *ebp);
static void plp_bb(pcm_context_t *ctxp, poid_t *svc_obj_p, int32 buy,
	char *tag, pin_errbuf_t *ebp);
...
static void tag_fn_catv(pcm_context_t *ctxp, poid_t *svc_obj_p,
	pin_flist_t *r_flistp, int32 *eidp, pin_errbuf_t *ebufp);
static void tag_fn_bb(pcm_context_t *ctxp, poid_t *svc_obj_p,
	pin_flist_t *r_flistp, int32 *eidp, pin_errbuf_t *ebufp);
...
static int32 valid_tag_ip(pcm_context_t *ctxp, poid_t *svc_obj_p,
	char *tag);
static int32 valid_tag_catv(pcm_context_t *ctxp, poid_t *svc_obj_p,
	char *tag);
static int32 valid_tag_bb(pcm_context_t *ctxp, poid_t *svc_obj_p,
	char *tag);
...
static struct service_info {
	...
	{ "/service/catv",  plp_catv,  tag_fn_catv,   valid_tag_catv },
	{ "/service/telco/broadband",  plp_bb,  tag_fn_bb,   valid_tag_bb },
	...
	pin_errbuf_t  *ebufp)
{
	...
	/* what to return */
	if (PIN_ERR_IS_ERR(ebufp)) {
		pin_snprintf(ebp, sizeof(ebp), 
			"error retrieving %s object", config_name);
...
/*
 * group of functions for /service/catv
 */
static void
tag_fn_catv(pcm_context_t *ctxp, poid_t *svc_obj_p, 
	pin_flist_t *r_flistp, int32 *eidp, pin_errbuf_t *ebufp)
{
	get_catv_provisioning_tags(ctxp, svc_obj_p, r_flistp, eidp, ebufp); 
}

static int32
valid_tag_catv(pcm_context_t *ctxp, poid_t *svc_obj_p, char *tag)
{
	if(isValidSSGTag(tag)) {
		return valid_tag_wireless(ctxp, svc_obj_p, tag, CATVCONFIG);
	} else {
		return(_valid_tag(tag, tags_catv));
	}
}

static void
plp_catv(pcm_context_t *ctxp, poid_t *svc_obj_p, int32 buy,
	char *tag, pin_errbuf_t *ebufp)
{
	if (strcmp(tag, "example") == 0) {
		plp_example(ctxp, svc_obj_p, buy, tag, ebufp);
	...
	{
		//plp_example(ctxp, svc_obj_p, buy, tag, ebufp);
		PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_DEBUG,"do CATV action here\n");
	}
//	else {
//		plp_ssg(ctxp, svc_obj_p, buy, tag, ebufp);
//	}
...
/*
 * group of functions for /service/broadband
 */
static void
tag_fn_bb(pcm_context_t *ctxp, poid_t *svc_obj_p, 
	pin_flist_t *r_flistp, int32 *eidp, pin_errbuf_t *ebufp)
{
	//_tag_fn(BBSERVICE, tags_bb, r_flistp, eidp, ebufp);
	//tag_fn_get_tags(ctxp, svc_obj_p, r_flistp, eidp, ebufp, BBCONFIG, BBSERVICE);
	get_bb_provisioning_tags(ctxp, svc_obj_p, r_flistp, eidp, ebufp);
}

static int32
valid_tag_bb(pcm_context_t *ctxp, poid_t *svc_obj_p, char *tag)
{
	if(isValidSSGTag(tag)) {
		return valid_tag_wireless(ctxp, svc_obj_p, tag, BBCONFIG);
	} else {
		return(_valid_tag(tag, tags_bb));
	}
}

static void
plp_bb(pcm_context_t *ctxp, poid_t *svc_obj_p, int32 buy,
	char *tag, pin_errbuf_t *ebufp)
{
	if (strcmp(tag, "example") == 0) {
		plp_example(ctxp, svc_obj_p, buy, tag, ebufp);
	...
	{
		//plp_example(ctxp, svc_obj_p, buy, tag, ebufp);
		PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_DEBUG,"do Broadband action here\n");
	}
//	else {
//		plp_ssg(ctxp, svc_obj_p, buy, tag, ebufp);
//	}
	...
	char *configtype, char *servicetype)
{
	...
	*eidp = eid;
         	...
         	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "catv provisioning tag return flistp", r_flp);
...
tag_fn_telephony(
        ...
        while ((a_flistp = PIN_FLIST_ELEM_GET_NEXT(c_flistp,
                ...
                if (feature != NULL && strcmp(feature, "")) {
                        ...
                        pin_snprintf(ctag_name, sizeof(ctag_name),
				 "%s;%s", product, feature);
	...
	)
{
	...
	if(strcmp(service_recd, PIN_OBJ_TYPE_SERVICE_GSM_TEL) &&
		...
		pin_snprintf(msg, sizeof(msg), 
			"tag_fn_gsm_xxx : Invalid Service Type : %s", service_recd);
	...
	if (c_flistp == (pin_flist_t *)NULL) {
		if (PIN_ERR_IS_ERR(ebufp)) {
			pin_snprintf(msg, sizeof(msg), 
				"Error retrieving gsm config object xxx : %s", config_recd);
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, msg, ebufp);
		}
		/*  Else, config object not found, but no error. */
		return;
	...
	while ((a_flistp = PIN_FLIST_ELEM_GET_NEXT(c_flistp,
		...
		if (tag_name == NULL || PIN_ERR_IS_ERR(ebufp)){
			PIN_FLIST_DESTROY(c_flistp, NULL);
			pin_snprintf(msg, sizeof(msg),
				 "gsm config object tag: missing PIN_FLD_PROVISIONING_TAG : %s",
				config_recd);
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, msg, ebufp);
			return;
		}
	...
	pin_errbuf_t *ebufp)
{
    ...
    char        *d_tag ="dummy";
	char		*template = "select X from /mso_cfg_catv_pt where F1 is not NULL ";
	/* get db number from the service poid */
	db_no = PIN_POID_GET_DB(svc_poidp);
	int32		flags = 256;
	servicetype = (char *)PIN_POID_GET_TYPE(svc_poidp);
    eid = *eidp;

	/* set up the search flist */
	s_flistp = PIN_FLIST_CREATE(ebufp);
	pdp = PIN_POID_CREATE(db_no, "/search", -1, ebufp);
	PIN_FLIST_FLD_PUT(s_flistp, PIN_FLD_POID, (void *)pdp, ebufp);
	PIN_FLIST_FLD_SET(s_flistp, PIN_FLD_TEMPLATE, template,ebufp);
	PIN_FLIST_FLD_SET(s_flistp, PIN_FLD_FLAGS, &flags,ebufp);

	flistp = PIN_FLIST_ELEM_ADD(s_flistp, PIN_FLD_ARGS, 1, ebufp);
	PIN_FLIST_FLD_SET(flistp, PIN_FLD_PROVISIONING_TAG,  d_tag, ebufp);
	...
	/* do the search */
    	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "get_catv_provisioning_tags provisioning tag search input flistp", s_flistp);
	PCM_OP(ctxp, PCM_OP_SEARCH, 0, s_flistp, &s_o_flistp, ebufp);
        	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "get_catv_provisioning_tags provisioning tag search output flistp", s_o_flistp);
	...
	if (PIN_ERRBUF_IS_ERR(ebufp)) {
		sprintf(ebp, "error retrieving /mso_cfg_catv_pt object");
                PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, ebp, ebufp);
		PIN_FLIST_DESTROY(s_o_flistp, NULL);
		return;
	}
	...
	while ((entry_flp = PIN_FLIST_ELEM_GET_NEXT(s_o_flistp,
		...
		/* Is everything groovy? */
		if (tag_name == NULL || PIN_ERRBUF_IS_ERR(ebufp)){
			/* No: clean up and exit */
			PIN_FLIST_DESTROY_EX(&s_o_flistp, NULL);

                	PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
                        	"get_catv_provisioning_tags function can't get "
				"PIN_FLD_PROVISIONING_TAG",
			 	ebufp);
			return;
		}
		...
		/* Add current tag to output */
		flp = PIN_FLIST_ELEM_ADD(r_flistp,
			PIN_FLD_PROD_PROVISIONING_TAGS, eid, ebufp);
		eid++;
		PIN_FLIST_FLD_SET(flp, PIN_FLD_TYPE_STR,
			CATVSERVICE, ebufp);
		PIN_FLIST_FLD_SET(flp, PIN_FLD_PROVISIONING_TAG,
			(void *)tag_name, ebufp);
         	...
         	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "get_catv_provisioning_tags return flistp", r_flistp);
	...
	pin_errbuf_t *ebufp)
{
    ...
    char        *servicetype = NULL;
    char        *d_tag ="dummy";
	char		*template = "select X from /mso_config_bb_pt where F1 is not NULL ";
	/* get db number from the service poid */
	db_no = PIN_POID_GET_DB(svc_poidp);
	int32		flags = 256;
	servicetype = (char *)PIN_POID_GET_TYPE(svc_poidp);
	...
	/* do the search */
    	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "get_bb_provisioning_tags provisioning tag search input flistp", s_flistp);
	PCM_OP(ctxp, PCM_OP_SEARCH, 0, s_flistp, &s_o_flistp, ebufp);
        	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "get_bb_provisioning_tags provisioning tag search output flistp", s_o_flistp);
	...
	/* what to return */
	if (PIN_ERRBUF_IS_ERR(ebufp)) {
		sprintf(ebp, "error retrieving /mso_config_bb_pt object");
                PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, ebp, ebufp);
		PIN_FLIST_DESTROY(s_o_flistp, NULL);
		return;
	}
	...
	while ((entry_flp = PIN_FLIST_ELEM_GET_NEXT(s_o_flistp,
		...
		if (tag_name == NULL || PIN_ERRBUF_IS_ERR(ebufp)){
			...
			PIN_FLIST_DESTROY_EX(&s_o_flistp, NULL);
                	...
                	PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
                        	"get_bb_provisioning_tags function can't get "
				"PIN_FLD_PROVISIONING_TAG",
			 	ebufp);
			return;
		}
		...
		PIN_FLIST_FLD_SET(flp, PIN_FLD_PROVISIONING_TAG,
			(void *)tag_name, ebufp);
         	...
         	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "get_bb_provisioning_tags return flistp", r_flistp);
