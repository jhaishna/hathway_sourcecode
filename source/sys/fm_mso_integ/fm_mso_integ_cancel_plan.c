/*******************************************************************
 * File:	fm_mso_integ_cancel_plan.c
 * Opcode:	MSO_OP_INTEG_CANCEL_PLAN 
 * Owner:	Rohini Mogili
 * Created:	21-NOV-2013
...
#ifndef lint
static const char Sccs_id[] = "@(#)%Portal Version: fm_mso_integ_cancel_plan.c:CUPmod7.3PatchInt:1:2006-Dec-01 16:31:18 %";
#endif
	...
	pin_errbuf_t		*ebufp)
{
	...
	*r_flistpp		= NULL;
	 ...
	 * Insanity Check 
	 *******************************************************************/
	if (opcode != MSO_OP_INTEG_CANCEL_PLAN) {
		pin_set_err(ebufp, PIN_ERRLOC_FM, 
			PIN_ERRCLASS_SYSTEM_DETERMINATE,
			PIN_ERR_BAD_OPCODE, 0, 0, opcode);
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
			"op_mso_integ_cancel_plan error",
			ebufp);
		return;
	}
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, 
		"op_mso_integ_cancel_plan input flist", i_flistp);
	...
	if(PIN_ERRBUF_IS_ERR(ebufp))
	{
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,"Err before calling fm_mso_integ_cancel_plan()", ebufp);
		return;
	}*/
	...
	fm_mso_integ_cancel_plan(ctxp, flags, i_flistp, r_flistpp, ebufp);
	...
	{		
		...
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,"op_mso_integ_cancel_plan error", ebufp);
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,	"op_mso_integ_cancel_plan output flist", *r_flistpp);
	return;
}
	...
	pin_errbuf_t		*ebufp)
{
	...
	if (PIN_ERRBUF_IS_ERR(ebufp))
		return;
	PIN_ERRBUF_CLEAR(ebufp);
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_cancel_plan input flist", i_flistp);	
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_cancel_plan search service input list", search_inflistp);
	...
	if (PIN_ERRBUF_IS_ERR(ebufp))
	{
		...
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_POID, PIN_FLIST_FLD_GET(i_flistp, PIN_FLD_POID, 0, ebufp ), ebufp );
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_STATUS, &cancel_plan_failure, ebufp);
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_CODE, "51618", ebufp);
		//PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_DESCR, "fm_mso_integ_cancel_plan search service error", ebufp);
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_DESCR, "search service error", ebufp);
		goto CLEANUP;
	}
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_cancel_plan search service output flist", search_outflistp);
	...
	if(results_flistp != NULL)
	{
		...
		PIN_FLIST_FLD_COPY(i_flistp, PIN_FLD_ACCOUNT_OBJ, canceldeal_flistp, PIN_FLD_POID, ebufp);
		PIN_FLIST_FLD_COPY(i_flistp, PIN_FLD_SERVICE_OBJ, canceldeal_flistp, PIN_FLD_SERVICE_OBJ, ebufp);
		PIN_FLIST_FLD_COPY(i_flistp, PIN_FLD_PROGRAM_NAME, canceldeal_flistp, PIN_FLD_PROGRAM_NAME, ebufp);
		PIN_FLIST_FLD_COPY(i_flistp, PIN_FLD_ACTION_NAME, canceldeal_flistp, PIN_FLD_ACTION_NAME, ebufp);
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_cancel_plan search plan input list", search_inflistp);
	PCM_OP(ctxp, PCM_OP_SEARCH, 0, search_inflistp, &search_outflistp, ebufp);
	PIN_FLIST_DESTROY_EX(&search_inflistp, NULL);
	if (PIN_ERRBUF_IS_ERR(ebufp))
	{
		...
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_POID, PIN_FLIST_FLD_GET(i_flistp, PIN_FLD_POID, 0, ebufp ), ebufp );
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_STATUS, &cancel_plan_failure, ebufp);
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_CODE, "51619", ebufp);
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_DESCR, "fm_mso_integ_cancel_plan search plan error", ebufp);
		goto CLEANUP;
	}
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_cancel_plan search plan output flist", search_outflistp);
	...
	if(!results_flistp)
	{
		ret_flistp = PIN_FLIST_CREATE(ebufp);
                ...
                PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_STATUS, &cancel_plan_failure, ebufp);
                PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_CODE, "51619", ebufp);
                PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_DESCR, "search plan error invalid plan name", ebufp);
                goto CLEANUP;
	...
	while ((deal_flistp = PIN_FLIST_ELEM_GET_NEXT(results_flistp, PIN_FLD_SERVICES, &elem_id, 1, &cookie, ebufp)) != NULL)
	{
		...
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_cancel_plan search plan output flist", search_deal_outflistp);
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_cancel_plan cancel plan input flist", canceldeal_flistp);
	...
	if (PIN_ERRBUF_IS_ERR(ebufp))
	{
		...
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_DESCR, "fm_mso_integ_cancel_plan cancel plan error", ebufp);
		goto CLEANUP;
	}
	...
	*******************************************************************/
	...
	{		
		PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_cancel_plan no need of cancel plan");	
	}
	else if ( acct_update == 2)
	{		
		PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_cancel_plan cancel plan successful");	
	}
	else if ( acct_update == 3)
	{		
		PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_cancel_plan insufficient data provided");	
	}*/	
