/*******************************************************************
 * File:	fm_mso_integ_suspend_service.c
 * Opcode:	MSO_OP_INTEG_SUSPEND_SERVICE 
 * Owner:	Rohini Mogili
 * Created:	21-NOV-2013
...
#ifndef lint
static const char Sccs_id[] = "@(#)%Portal Version: fm_mso_integ_suspend_service.c:CUPmod7.3PatchInt:1:2006-Dec-01 16:31:18 %";
#endif
	...
	pin_errbuf_t		*ebufp)
{
	...
	*r_flistpp		= NULL;
	 ...
	 * Insanity Check 
	 *******************************************************************/
	if (opcode != MSO_OP_INTEG_SUSPEND_SERVICE) {
		pin_set_err(ebufp, PIN_ERRLOC_FM, 
			PIN_ERRCLASS_SYSTEM_DETERMINATE,
			PIN_ERR_BAD_OPCODE, 0, 0, opcode);
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
			"op_mso_integ_suspend_service error",
			ebufp);
		return;
	}
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, 
		"op_mso_integ_suspend_service input flist", i_flistp);
	...
	if(PIN_ERRBUF_IS_ERR(ebufp))
	{
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,"Err before calling fm_mso_integ_suspend_service()", ebufp);
		return;
	}*/
...
//	if (!(prog_name && strcmp(prog_name,"pin_deferred_act") ==0 ))
	...
	fm_mso_integ_suspend_service(ctxp, flags, i_flistp, r_flistpp, ebufp);
	...
	{		
		...
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,"op_mso_integ_suspend_service error", ebufp);
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,	"op_mso_integ_suspend_service output flist", *r_flistpp);
	return;
}
	...
	pin_errbuf_t		*ebufp)
{
	...
	if (PIN_ERRBUF_IS_ERR(ebufp))
		return;
	PIN_ERRBUF_CLEAR(ebufp);
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_suspend_service input flist", i_flistp);	
...
//
//	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_suspend_service search service input list", search_inflistp);
...
//		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_DESCR, "fm_mso_integ_suspend_service search service error", ebufp);
//		goto CLEANUP;
//	}
//	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_suspend_service search service output flist", search_outflistp);
//
...
//	PIN_FLIST_FLD_COPY(i_flistp, PIN_FLD_ACCOUNT_OBJ, change_status_inflistp, PIN_FLD_POID, ebufp);
//	PIN_FLIST_FLD_COPY(i_flistp, PIN_FLD_SERVICE_OBJ, change_status_inflistp, PIN_FLD_SERVICE_OBJ, ebufp);
...
//	PIN_FLIST_FLD_COPY(i_flistp, PIN_FLD_DESCR, change_status_inflistp, PIN_FLD_DESCR, ebufp);
	//PIN_FLIST_FLD_SET(change_status_inflistp, PIN_FLD_DESCR, "Serivce status change", ebufp);
...
//		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_DESCR, "fm_mso_integ_suspend_service reactivation error", ebufp);
//		goto CLEANUP;
//	}

	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_suspend_service reactivation input flist", change_status_inflistp);

	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_integ_add_plan search service suspend input list", search_inflistp);
	PCM_OP(ctxp, MSO_OP_CUST_SUSPEND_SERVICE, 0, i_flistp, &ret_flistp, ebufp);



	//result_status = fm_mso_suspend_service_status(ctxp, i_flistp, &ret_flistp,  ebufp);
	
	if (PIN_ERRBUF_IS_ERR(ebufp))
	{
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "Error in calling MSO_OP_CUST_SUSPEND_SERVICE", ebufp);
		PIN_ERRBUF_RESET(ebufp);
		ret_flistp = PIN_FLIST_CREATE(ebufp);
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_POID, PIN_FLIST_FLD_GET(i_flistp, PIN_FLD_POID, 0, ebufp ), ebufp );
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_STATUS, &status_change_failure, ebufp);
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_CODE, "51631", ebufp);
		PIN_FLIST_FLD_SET(ret_flistp, PIN_FLD_ERROR_DESCR, "Error in calling MSO_OP_CUST_SUSPEND_SERVICE", ebufp);
		goto CLEANUP;
	}
