...
*--------------------------------------------------------------------------------------------------------------*
* 0.1    |25/10/2013 |Satya Prakash     |  CATV Fallover Management
*--------------------------------------------------------------------------------------------------------------*
*
****************************************************************************************************************/
...
#ifndef lint
static  char Sccs_Id[] = "@(#)$Id: fm_mso_prov_resubmit_failed_order.c /cgbubrm_7.3.2.rwsmod/1 2009/08/04 23:37:19 pjegan Exp $";
#endif
...
EXPORT_OP void
op_mso_prov_resubmit_failed_order(
...
static void
fm_mso_prov_resubmit_failed_order(
        pcm_context_t       *ctxp,
        pin_flist_t         *in_flistp,
        int32               flags,
        ...
        pin_errbuf_t        *ebufp)
{
    ...
    *ret_flistpp = NULL;
     ...
     * Insanity check.
     ***********************************************************/
    if (opcode != MSO_OP_PROV_RESUBMIT_FAILED_ORDER) {
        pin_set_err(ebufp, PIN_ERRLOC_FM,
            PIN_ERRCLASS_SYSTEM_DETERMINATE,
                PIN_ERR_BAD_OPCODE, 0, 0, opcode);
        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
            "bad opcode in op_mso_prov_resubmit_failed_order", ebufp);
        return;
    }
    ...
    PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
        "op_mso_prov_resubmit_failed_order input flist", in_flistp);
     ...
     ***********************************************************/
    fm_mso_prov_resubmit_failed_order(ctxp, in_flistp, flags, &r_flistp, ebufp);
    ...
    {
        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
            "op_mso_prov_resubmit_failed_order error", ebufp);
    ...
    PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
        "op_mso_prov_resubmit_failed_order return flist", *ret_flistpp);
    ...
    pin_errbuf_t        *ebufp)
{
     ...
     ***********************************************************/
    PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
        "fm_mso_prov_resubmit_failed_order input flist", in_flistp);
    ...
    if (PIN_ERRBUF_IS_ERR(ebufp) || task_count == 0 ) {
        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
            "fm_mso_prov_resubmit_failed_order error: required field missing in input flist", ebufp);
        return;
    }
    ...
    for (i = 1;i < *failed_task_id ;i++ )
    {
        ...
        PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                "fm_mso_prov_resubmit_failed_order input flist", payload_flistp);
        if (task_flistp)
        {
    ...
    PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
        "fm_mso_prov_resubmit_failed_order create input flist", order_i_flistp);
    PCM_OP(ctxp, PCM_OP_CREATE_OBJ, 0, order_i_flistp, out_flistpp, ebufp);
    PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
        "fm_mso_prov_resubmit_failed_order order create output flist", *out_flistpp);

    
    PIN_FLIST_FLD_SET(*out_flistpp, PIN_FLD_ORDER_ID, new_order_id, ebufp);
    /**
     * Search for old order object
     */
    search_i_flistp = PIN_FLIST_CREATE(ebufp);
    database = PIN_POID_GET_DB(acc_poidp);
    ...
    strcpy(template, "select X from /mso_prov_order where F1 = V1");
    PIN_FLIST_FLD_SET(search_i_flistp, PIN_FLD_TEMPLATE, template, ebufp);
    t_flistp = PIN_FLIST_ELEM_ADD(search_i_flistp, PIN_FLD_ARGS, 1, ebufp);
    PIN_FLIST_FLD_SET(t_flistp, PIN_FLD_ORDER_ID, old_order_id, ebufp);

    t_flistp = PIN_FLIST_ELEM_ADD (search_i_flistp, PIN_FLD_RESULTS, PIN_ELEMID_ANY, ebufp);
    PIN_FLIST_FLD_SET(t_flistp, PIN_FLD_POID, NULL, ebufp);
    PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                        "fm_mso_prov_resubmit_failed_order PCM_OP_SEARCH input flist", search_i_flistp);
    PCM_OP(ctxp, PCM_OP_SEARCH, 0, search_i_flistp, &search_o_flistp, ebufp);
    PIN_FLIST_DESTROY_EX(&search_i_flistp, NULL);
    PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                        "fm_mso_prov_resubmit_failed_order PCM_OP_SEARCH output flist", search_o_flistp);

    t_flistp = PIN_FLIST_ELEM_GET(search_o_flistp, PIN_FLD_RESULTS, PIN_ELEMID_ANY, 1,ebufp);
    old_order_pdp = PIN_FLIST_FLD_GET(t_flistp, PIN_FLD_POID, 0, ebufp);
    ...
    PIN_FLIST_FLD_SET(w_i_flistp, PIN_FLD_POID, old_order_pdp, ebufp);
    PIN_FLIST_FLD_SET(w_i_flistp, PIN_FLD_STATUS, &status, ebufp); //CORRECTED

    PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
        "fm_mso_prov_resubmit_failed_order update previous order status input flist", w_i_flistp);
    PCM_OP(ctxp, PCM_OP_WRITE_FLDS, 0, w_i_flistp, &w_o_flistp, ebufp);
    PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
        "fm_mso_prov_resubmit_failed_order update previous order status output flist", w_o_flistp);

	lc_in_flist  = PIN_FLIST_CREATE(ebufp);
	PIN_FLIST_FLD_SET(lc_in_flist, PIN_FLD_ACCOUNT_OBJ, acc_poidp, ebufp );
	PIN_FLIST_FLD_SET(lc_in_flist, PIN_FLD_ORDER_ID, acc_poidp, ebufp );
