...
/*******************************************************************
 * File:        fm_mso_commission_rectify_lco_wrong_tagging.c
 * Opcode:      MSO_OP_COMMISSION_RECTIFY_LCO_WRONG_TAGGING
 * Description:
 *
...
#ifndef lint
static const char Sccs_id[] = "@(#)%Portal Version: fm_mso_commission_rectify_lco_wrong_tagging.c:CUPmod7.3PatchInt:1:2006-Dec-01 16:31:18 %";
#endif
...
EXPORT_OP void
op_mso_commission_rectify_lco_wrong_tagging (
...
static void
fm_mso_commission_rectify_lco_wrong_tagging (
        pcm_context_t            *ctxp,
        int32                   flags,
        pin_flist_t             *i_flistp,
        pin_flist_t             **r_flistpp,
...
static void
fm_mso_commission_get_all_commission_charges(
        pcm_context_t           *ctxp,
	int32			flags,
        pin_flist_t             *i_flistp,
...
static void
fm_mso_commission_revert_lco_commission(
        pcm_context_t           *ctxp,
        pin_flist_t             *i_flistp,
        pin_flist_t             **err_flistp,
        pin_errbuf_t            *ebufp);
...
static void
fm_mso_commission_revert_sdt_commission(
        pcm_context_t           *ctxp,
        pin_flist_t             *i_flistp,
        pin_flist_t             **err_flistp,
        pin_errbuf_t            *ebufp);
...
static void
fm_mso_commission_revert_dtr_commission(
        pcm_context_t           *ctxp,
        pin_flist_t             *i_flistp,
        pin_flist_t             **err_flistp,
        pin_errbuf_t            *ebufp);
...
static void
fm_mso_commission_apply_commission_for_new_lco(
        pcm_context_t           *ctxp,
        pin_flist_t             *i_flistp,
        pin_flist_t             *old_lco_comm_flistp,
        pin_flist_t             **err_flistp,
...
static void
fm_mso_commission_apply_commission_for_new_sdt(
        pcm_context_t           *ctxp,
        pin_flist_t             *i_flistp,
        pin_flist_t             *old_lco_comm_flistp,
        pin_flist_t             **err_flistp,
...
static void
fm_mso_commission_apply_commission_for_new_dtr(
        pcm_context_t           *ctxp,
        pin_flist_t             *i_flistp,
        pin_flist_t             *old_lco_comm_flistp,
        pin_flist_t             **err_flistp,
...
op_mso_commission_rectify_lco_wrong_tagging (
        ...
        *r_flistpp              = NULL;
         ...
         * Insanity Check
         *******************************************************************/
        if (opcode != MSO_OP_COMMISSION_RECTIFY_LCO_WRONG_TAGGING) {
                pin_set_err(ebufp, PIN_ERRLOC_FM,
                        PIN_ERRCLASS_SYSTEM_DETERMINATE,
                        PIN_ERR_BAD_OPCODE, 0, 0, opcode);
                PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
                        "op_mso_commission_rectify_lco_wrong_tagging error",ebufp);
                return;
        }
        /*******************************************************************
         ...
         * Debug: Input flist
         *******************************************************************/
        PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "op_mso_commission_rectify_lco_wrong_tagging input flist ", i_flistp);

        fm_mso_commission_rectify_lco_wrong_tagging(ctxp, flags, i_flistp, r_flistpp, ebufp);

        PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "op_mso_commission_rectify_lco_wrong_tagging output flist", *r_flistpp);
...
//	int32		ret_val = 0;
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_rectify_lco_wrong_tagging input flist", i_flistp);

	/*: Get all the commission belongs to the old lco account START*/
	fm_mso_commission_get_all_commission_charges(ctxp, flags, i_flistp, &all_comm_flistp, &err_flistp, ebufp);
	if (PIN_ERRBUF_IS_ERR(ebufp)) 
	{
	...
	if (all_comm_flistp)
	{
		old_lco_comm_flistp = PIN_FLIST_ELEM_GET(all_comm_flistp, MSO_FLD_LCOINFO, 0, 1, ebufp);
	...
	if (old_lco_comm_flistp)
	{
		lco_results = PIN_FLIST_ELEM_GET(old_lco_comm_flistp, PIN_FLD_RESULTS, PIN_ELEMID_ANY, 1, ebufp );
	}
	...
	*********************/
	...
	if(lco_results) 
	{
		PIN_FLIST_FLD_COPY(i_flistp, PIN_FLD_PROGRAM_NAME, old_lco_comm_flistp, PIN_FLD_PROGRAM_NAME, ebufp);
		fm_mso_commission_revert_lco_commission(ctxp, old_lco_comm_flistp, &err_flistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) 
		{
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_revert_lco_commission: Write fields error",ebufp);
	...
	if(lco_results) 
	{
		fm_mso_commission_apply_commission_for_new_lco(ctxp, i_flistp, old_lco_comm_flistp, &err_flistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) 
		{
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_apply_commission_for_new_lco: Write fields error",ebufp);
			PIN_ERRBUF_RESET(ebufp);
			goto CLEANUP;
		}
	}
	...
	*********************/
	...
	if(sdt_results) 
	{
		...
		fm_mso_commission_revert_sdt_commission(ctxp, old_sdt_comm_flistp, &err_flistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) 
		{
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_revert_sdt_commission: Write fields error",ebufp);
			PIN_ERRBUF_RESET(ebufp);
			goto CLEANUP;
		}
	}
	...
	if(sdt_results) 
	{
		fm_mso_commission_apply_commission_for_new_sdt(ctxp, i_flistp, old_sdt_comm_flistp, &err_flistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) 
		{
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_apply_commission_for_new_sdt: Write fields error",ebufp);
			PIN_ERRBUF_RESET(ebufp);
			goto CLEANUP;
		}
	}
	...
	*********************/
	...
	if(dtr_results) 
	{
		...
		fm_mso_commission_revert_dtr_commission(ctxp, old_dtr_comm_flistp, &err_flistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) 
		{
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_revert_dtr_commission: Write fields error",ebufp);
			PIN_ERRBUF_RESET(ebufp);
			goto CLEANUP;
		}
	}
	...
	if(dtr_results) 
	{
		fm_mso_commission_apply_commission_for_new_dtr(ctxp, i_flistp, old_dtr_comm_flistp, &err_flistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) 
		{
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_apply_commission_for_new_dtr: Write fields error",ebufp);
			PIN_ERRBUF_RESET(ebufp);
			goto CLEANUP;
		}
	}
...
//	int			tmp_elem_id = 0;
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_lco_commission input flist", i_flistp);
	...
	while ((src_res_flistp = PIN_FLIST_ELEM_GET_NEXT(i_flistp, PIN_FLD_RESULTS, &elem_id, 1, &cookie, ebufp )) != NULL) {
		...
		PIN_FLIST_FLD_COPY(src_res_flistp,MSO_FLD_LCO_OBJ,adj_evt_i_flistp,PIN_FLD_POID,ebufp);
		...
		//PIN_FLIST_FLD_SET(adj_evt_i_flistp,PIN_FLD_PROGRAM_NAME,ADJ_PRG_NAME,ebufp);
		PIN_FLIST_FLD_COPY(i_flistp, PIN_FLD_PROGRAM_NAME, adj_evt_i_flistp, PIN_FLD_PROGRAM_NAME, ebufp);
		PIN_FLIST_FLD_SET(adj_evt_i_flistp,PIN_FLD_DESCR,ADJ_PRG_NAME,ebufp);
		PIN_FLIST_FLD_SET(adj_evt_i_flistp,PIN_FLD_SYS_DESCR,ADJ_PRG_NAME,ebufp);
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_lco_commission  OOB adjustment input flist", adj_evt_i_flistp);
		PCM_OP(ctxp, PCM_OP_AR_EVENT_ADJUSTMENT, 0, adj_evt_i_flistp, &adj_evt_o_flistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) {
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
			"fm_mso_commission_rectify_lco_wrong_tagging Event Adjustment error",ebufp);
			return;
		}
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_lco_commission  OOB adjustment output flist", adj_evt_o_flistp);
		...
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_lco_commission: Write fields input flist", wrtflds_iflistp);
		...
		PIN_FLIST_FLD_SET(rev_comm_flistp, PIN_FLD_STATUS_MSG, "Commission Reversed Due to LCO Wrong Tagging", ebufp);

                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_lco_commission: Create input flist", rev_comm_flistp);
                PCM_OP(ctxp, PCM_OP_CREATE_OBJ, 0, rev_comm_flistp, &r_rev_comm_flistp, ebufp);
                if (PIN_ERRBUF_IS_ERR(ebufp)) {
                        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_revert_lco_commission: Create error",ebufp);
                        return;
                }
                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_lco_commission: Create output flist", r_rev_comm_flistp);
...
//	int			tmp_elem_id = 0;
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_sdt_commission input flist", i_flistp);
	...
	while ((src_res_flistp = PIN_FLIST_ELEM_GET_NEXT(i_flistp, PIN_FLD_RESULTS, &elem_id, 1, &cookie, ebufp )) != NULL) 
	{
		...
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_sdt_commission  OOB adjustment input flist", adj_evt_i_flistp);
		PCM_OP(ctxp, PCM_OP_AR_EVENT_ADJUSTMENT, 0, adj_evt_i_flistp, &adj_evt_o_flistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) {
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
		...
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_sdt_commission  OOB adjustment output flist", adj_evt_o_flistp);

		//To Update the status of commission report to 999(Unique status)
		wrtflds_iflistp = PIN_FLIST_CREATE(ebufp);
		PIN_FLIST_FLD_COPY(src_res_flistp,PIN_FLD_POID,wrtflds_iflistp,PIN_FLD_POID,ebufp);
		PIN_FLIST_FLD_SET(wrtflds_iflistp, PIN_FLD_STATUS, &status, ebufp);
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_sdt_commission: Write fields input flist", wrtflds_iflistp);
		PCM_OP(ctxp, PCM_OP_WRITE_FLDS, 0, wrtflds_iflistp, &wrtflds_oflistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) {
                        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_revert_sdt_commission: Write fields error",ebufp);
			PIN_FLIST_DESTROY_EX(&wrtflds_iflistp,ebufp);
		...
		PIN_FLIST_FLD_SET(rev_comm_flistp, PIN_FLD_STATUS_MSG, "Commission Reversed Due to SDT Wrong Tagging", ebufp);
                ...
                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_sdt_commission: Create input flist", rev_comm_flistp);
                PCM_OP(ctxp, PCM_OP_CREATE_OBJ, 0, rev_comm_flistp, &r_rev_comm_flistp, ebufp);
                if (PIN_ERRBUF_IS_ERR(ebufp)) {
                        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_revert_sdt_commission: Create error",ebufp);
                        return;
                }
                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_sdt_commission: Create output flist", r_rev_comm_flistp);
		PIN_FLIST_DESTROY_EX(&rev_comm_flistp,ebufp);
		PIN_FLIST_DESTROY_EX(&r_rev_comm_flistp,ebufp);
...
//	int			tmp_elem_id = 0;
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_dtr_commission input flist", i_flistp);
	...
	while ((src_res_flistp = PIN_FLIST_ELEM_GET_NEXT(i_flistp, PIN_FLD_RESULTS, &elem_id, 1, &cookie, ebufp )) != NULL) {
		...
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_dtr_commission  OOB adjustment output flist", adj_evt_o_flistp);

		//To Update the status of commission report to 999(Unique status)
		wrtflds_iflistp = PIN_FLIST_CREATE(ebufp);
		PIN_FLIST_FLD_COPY(src_res_flistp,PIN_FLD_POID,wrtflds_iflistp,PIN_FLD_POID,ebufp);
		PIN_FLIST_FLD_SET(wrtflds_iflistp, PIN_FLD_STATUS, &status, ebufp);
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_dtr_commission: Write fields input flist", wrtflds_iflistp);
		PCM_OP(ctxp, PCM_OP_WRITE_FLDS, 0, wrtflds_iflistp, &wrtflds_oflistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) {
                        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_revert_dtr_commission: Write fields error",ebufp);
			PIN_FLIST_DESTROY_EX(&wrtflds_iflistp,ebufp);
		...
		PIN_FLIST_FLD_SET(rev_comm_flistp, PIN_FLD_STATUS_MSG, "Commission Reversed Due to DTR Wrong Tagging", ebufp);
                ...
                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_dtr_commission: Create input flist", rev_comm_flistp);
                PCM_OP(ctxp, PCM_OP_CREATE_OBJ, 0, rev_comm_flistp, &r_rev_comm_flistp, ebufp);
                if (PIN_ERRBUF_IS_ERR(ebufp)) {
                        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_revert_dtr_commission: Create error",ebufp);
                        return;
                }
                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_revert_dtr_commission: Create output flist", r_rev_comm_flistp);
		PIN_FLIST_DESTROY_EX(&rev_comm_flistp,ebufp);
		PIN_FLIST_DESTROY_EX(&r_rev_comm_flistp,ebufp);
...
//	int32		acnt_type_dtr = ACCT_TYPE_DTR;
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_get_all_commission_charges input flist", i_flistp);

        /*: Get both LCO accounts START*/
        cust_acct_objp = PIN_FLIST_FLD_GET(i_flistp, PIN_FLD_POID, 0, ebufp);
	...
	**************************************/
	...
	if (new_dt_objp && old_dt_objp && PIN_POID_COMPARE(new_dt_objp, old_dt_objp, 0, ebufp)!=0 )
	{
        ...
        PIN_ERR_LOG_POID(PIN_ERR_LEVEL_DEBUG, "Customer Account POID", cust_acct_objp);
        PIN_ERR_LOG_POID(PIN_ERR_LEVEL_DEBUG, "Source (old) LCO Account POID", curr_lco_objp);
	...
	/*: Get both LCO accounts END*/
        ...
        PIN_FLIST_FLD_SET(args_flistp,MSO_FLD_LCO_OBJ, curr_lco_objp, ebufp);

        args_flistp = PIN_FLIST_ELEM_ADD(s_flistp, PIN_FLD_ARGS, 2, ebufp);
        PIN_FLIST_FLD_SET(args_flistp, PIN_FLD_ACCOUNT_OBJ, cust_acct_objp, ebufp);
...
//	}
        ...
        PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_get_all_commission_charges get old commission search flist", s_flistp);
        PCM_OP(ctxp, PCM_OP_SEARCH, 0, s_flistp, &r_s_flistp, ebufp);
        if (PIN_ERRBUF_IS_ERR(ebufp)) {
                PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "fm_mso_commission_get_all_commission_charges get old commissions search error",ebufp);
                return;
        }
        PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_get_all_commission_charges get old commissions output flist", r_s_flistp);
	...
	*all_comm_flistp = PIN_FLIST_CREATE(ebufp);
	...
	if (r_s_flistp)
	{
			...
			&elem_id, 1, &cookie, ebufp )) != NULL) 
		{
			...
			if (comm_for && *comm_for == ACCT_TYPE_LCO)
			{
				PIN_FLIST_ELEM_COPY(r_s_flistp, PIN_FLD_RESULTS, elem_id, lco_comm_flistp, PIN_FLD_RESULTS, elem_id, ebufp);
			}
			else if (comm_for && *comm_for == ACCT_TYPE_SUB_DTR && sdt_is_different )
			{
				PIN_FLIST_ELEM_COPY(r_s_flistp, PIN_FLD_RESULTS, elem_id, sdt_comm_flistp, PIN_FLD_RESULTS, elem_id, ebufp);
			}
			else if (comm_for && *comm_for == ACCT_TYPE_DTR && dt_is_different)
			{
...
//	poid_t			*lco_acct_objp = NULL;
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_apply_commission_for_new_lco input1 flist", i_flistp);
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_apply_commission_for_new_lco input2 flist", old_lco_comm_flistp);

	/*: Get the new lco account poid START*/
	acct_flistp = PIN_FLIST_ELEM_GET(i_flistp, PIN_FLD_ACCOUNTS, 1, 0, ebufp);
	new_lco_objp = PIN_FLIST_FLD_GET(acct_flistp, PIN_FLD_POID, 0, ebufp);
	/*: Get the new lco account poid END*/

	/*: Get the commission model of the new lco account START*/
	if (fm_mso_commission_get_vald_lco_acct_comm_mdl (ctxp, i_flistp, &lco_flistp, ebufp) == 0) 
	{
	...
	/*: Get the commission model of the new lco account END*/

	/*: Verify the new LCO's commission bucket START*/
	fm_mso_utils_get_bal_grp(ctxp, new_lco_objp, COMMISSION_BUCKET, &lco_comm_grp_objp, &lco_outcollect_serv_objp, ebufp);
	if (lco_comm_grp_objp == NULL) 
	{
		fm_mso_utils_prepare_error_flist(ctxp, lco_flistp, err_flistp, "61053", "COMMISSION - Commission Bucket not found",ebufp);
	...
	PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_DEBUG, "LCO Commission Balance Group Buckets Retrieved");
	/*: Verify the new LCO's commission bucket END*/

//	/*: Verify the new LCO's commission bucket END*/
//	ret_val = fm_mso_commission_acct_has_agreement(ctxp,  new_lco_objp, ebufp);
//	if (ret_val == 0)  // Not having active agreement
//	{ 
//		if (commission_model != NULL && *commission_model != 0) 
//		{
//			fm_mso_utils_prepare_error_flist(ctxp, lco_flistp, err_flistp, "61054", "COMMISSION - New lco has no active agreement",ebufp);
//			fm_mso_commission_record_failure(ctxp, "/event/billing/settlement/ws/outcollect/charge_head_based", i_flistp, *err_flistp, ebufp);
...
//	/*: Verify the new LCO's commission bucket END*/
	...
	while ((old_comm_flistp = PIN_FLIST_ELEM_GET_NEXT(old_lco_comm_flistp, PIN_FLD_RESULTS, 
		&elem_id, 1, &cookie, ebufp )) != NULL) 
	{
		...
		PIN_FLIST_FLD_COPY(old_comm_flistp, MSO_FLD_DT_OBJ, new_comm_flistp, MSO_FLD_DT_OBJ, ebufp);
		PIN_FLIST_FLD_COPY(old_comm_flistp, MSO_FLD_SDT_OBJ, new_comm_flistp, MSO_FLD_SDT_OBJ, ebufp);
		...
		PIN_FLIST_FLD_SET(new_comm_flistp, MSO_FLD_LCO_COMM_BUCKET_OBJ, lco_comm_grp_objp, ebufp);
		PIN_FLIST_FLD_SET(new_comm_flistp, MSO_FLD_LCO_OUTCOLLECT_SERV_OBJ, lco_outcollect_serv_objp, ebufp);
		...
		PCM_OP(ctxp,MSO_OP_COMMISSION_PROCESS_COMMISSION, LCO_COMMISSION, new_comm_flistp, &r_new_comm_flistp, ebufp);
		if (PIN_ERRBUF_IS_ERR(ebufp)) 
		{
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
				"fm_mso_commission_apply_commission_for_new_lco: Process Commission opcode error", ebufp);
			return;
		}
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_apply_commission_for_new_lco output flist", i_flistp);
...
//	poid_t			*sdt_acct_objp = NULL;
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_apply_commission_for_new_sdt input1 flist", i_flistp);
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_apply_commission_for_new_sdt input2 flist", old_sdt_comm_flistp);
...
//	/*: Verify the new LCO's commission bucket END*/
		...
		&elem_id, 1, &cookie, ebufp )) != NULL) 
	{
		...
		{
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
				"fm_mso_commission_apply_commission_for_new_sdt: Process Commission opcode error", ebufp);
			return;
		}
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_apply_commission_for_new_sdt output flist", i_flistp);
...
//	poid_t			*dtr_acct_objp = NULL;
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_apply_commission_for_new_dtr input1 flist", i_flistp);
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_apply_commission_for_new_dtr input2 flist", old_dtr_comm_flistp);
...
//	/*: Verify the new LCO's commission bucket END*/
		...
		&elem_id, 1, &cookie, ebufp )) != NULL) 
	{
		...
		{
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
				"fm_mso_commission_apply_commission_for_new_dtr: Process Commission opcode error", ebufp);
			return;
		}
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_commission_apply_commission_for_new_dtr output flist", i_flistp);
