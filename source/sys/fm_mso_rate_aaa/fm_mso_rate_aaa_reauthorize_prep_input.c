/*******************************************************************
 * File:	fm_mso_rate_aaa_reauthorize_prep_input.c
 * Opcode:	MSO_OP_RATE_AAA_REAUTHORIZE_PREP_INPUT
...
#ifndef lint
static const char Sccs_id[] = "@(#)%Portal Version: fm_mso_rate_aaa_reauthorize_prep_input.c:CUPmod7.3PatchInt:1:2006-Dec-01 16:31:18 %";
#endif
...
EXPORT_OP void
op_mso_rate_aaa_reauthorize_prep_input(
...
static void
fm_mso_rate_aaa_reauthorize_prep_input(
	pcm_context_t		*ctxp,
	int32			flags,
	pin_flist_t		*i_flistp,
	pin_flist_t		**r_flistpp,
	...
	pin_errbuf_t		*ebufp)
{
	 ...
	 * Insanity Check 
	 *******************************************************************/
	if (opcode != MSO_OP_RATE_AAA_REAUTHORIZE_PREP_INPUT) {
		pin_set_err(ebufp, PIN_ERRLOC_FM, 
			PIN_ERRCLASS_SYSTEM_DETERMINATE,
			PIN_ERR_BAD_OPCODE, 0, 0, opcode);
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
			"op_mso_rate_aaa_reauthorize_prep_input error",
			ebufp);
		return;
	}
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, 
		"op_mso_rate_aaa_reauthorize_prep_input input flist", i_flistp);
	
	/*******************************************************************
	 * Call the default implementation 
	 *******************************************************************/
	fm_mso_rate_aaa_reauthorize_prep_input(ctxp, flags, i_flistp, &r_flistp, ebufp);
 	/***********************************************************
	 * Results.
	 ***********************************************************/
        ...
        if (PIN_ERRBUF_IS_ERR(ebufp)) {
                *r_flistpp = (pin_flist_t *)NULL;
                ...
                PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
                        "op_mso_rate_aaa_reauthorize_prep_input error", ebufp);
        } else {
                *r_flistpp = r_flistp;
                PIN_ERRBUF_CLEAR(ebufp);
                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                        "op_mso_rate_aaa_reauthorize_prep_input return flist", *r_flistpp);
	...
	pin_errbuf_t		*ebufp)
{
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_rate_aaa_reauthorize_prep_input input Flist", i_flistp);
	/*
	...
	if (PIN_ERRBUF_IS_ERR(ebufp)) {
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,"fm_mso_rate_aaa_reauthorize_prep_input error",ebufp);
		return;
	}
	...
	if(operation_type == 4){
        ...
        PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_rate_aaa_reauthorize_prep_input after drop session info input Flist", i_flistp);


	}

	//call PCM_OP_TCF_AAA_REAUTHORIZE_PREP_INPUT

	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_rate_aaa_reauthorize_prep_input before helper opcode call input Flist", i_flistp);
	PCM_OP(ctxp, PCM_OP_TCF_AAA_REAUTHORIZE_PREP_INPUT, 0, i_flistp, r_flistpp, ebufp);
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_rate_aaa_reauthorize_prep_input after helper opcode call return Flist", i_flistp);
