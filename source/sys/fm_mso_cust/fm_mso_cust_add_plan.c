	...
	pin_errbuf_t		*ebufp)
{
	...
	pin_flist_t	*base_res_flistp = NULL;
	pin_flist_t	*disc_flistp = NULL;
	pin_flist_t 	*susb_inflistp = NULL;
	pin_flist_t 	*susb_oflistp = NULL;
				...
				else
				{
					...
					if (PIN_ERRBUF_IS_ERR(ebufp)) 
					{
						...
						PIN_FLIST_FLD_SET(r_flistp, PIN_FLD_STATUS, &add_plan_failure, ebufp);
                        			...
                        			{
                            				PIN_FLIST_FLD_SET(r_flistp, PIN_FLD_ERROR_DESCR,
								"Service renew plan - There is no sufficient balance", ebufp);
                        			}
                        			else
                        			{
							PIN_FLIST_FLD_SET(r_flistp, PIN_FLD_ERROR_DESCR, "Renewal failed : ", ebufp);		
					...
					if (prod_oflist)
					{
            					results_flistp = PIN_FLIST_ELEM_GET(prod_oflist, PIN_FLD_RESULTS, 1, 1, ebufp);
						if (results_flistp == NULL)
						{
							susb_inflistp = PIN_FLIST_CREATE(ebufp);
							PIN_FLIST_FLD_COPY(prod_oflist, PIN_FLD_POID, susb_inflistp, PIN_FLD_POID, ebufp);
							PIN_FLIST_FLD_SET(susb_inflistp, PIN_FLD_PROGRAM_NAME, "pin_cycle_fees", ebufp);
							PIN_FLIST_FLD_SET(susb_inflistp, PIN_FLD_FLAGS, &srch_flags, ebufp );
							PIN_FLIST_FLD_SET(susb_inflistp, PIN_FLD_END_T, &current_t, ebufp );
				 			PCM_OP(ctxp, PCM_OP_SUBSCRIPTION_CYCLE_FORWARD, 0, susb_inflistp, &susb_oflistp, ebufp);	
							PIN_FLIST_DESTROY_EX(&susb_inflistp, NULL);
							if (PIN_ERRBUF_IS_ERR(ebufp))
							{			
								PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "Error in PCM_OP_SUBSCRIPTION_CYCLE_FORWARD ", ebufp);
                        					field_no = ebufp->field;
                        					errorCode = ebufp->pin_err;
								PIN_ERRBUF_RESET(ebufp);
								r_flistp = PIN_FLIST_CREATE(ebufp);
								...
								PIN_FLIST_FLD_SET(r_flistp, PIN_FLD_STATUS, &add_plan_failure, ebufp);
									...
									"Service renew plan - There is no sufficient balance", ebufp);
                        					...
                        					{
									PIN_FLIST_FLD_SET(r_flistp, PIN_FLD_ERROR_DESCR, 
										"Renewal failed due to charge error: ", ebufp);		
                        					}
								*ret_flistp = r_flistp;
								goto CLEANUP;
							}
							PIN_FLIST_DESTROY_EX(&susb_oflistp, NULL);
						}
			  		}
...
NOMATCH:
		...
		if (  prov_counter == 1 )
		{
			...
			if (ADD_MB_DATA_OR_FUP_FLAG != 1)
			{
				PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_DEBUG,"FUP CHECK");
