	...
	pin_errbuf_t		*ebufp)
{
	...
	{		
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_ERROR,
                        "op_mso_cust_bb_hw_amc Failed input flist", i_flistp);
...

{
	...
	eve_type = PIN_FLIST_FLD_GET(s_rflistp, PIN_FLD_EVENT_TYPE, 0, ebufp);
        ...
        {
                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_ERROR,
                        "amc_parse_input ERROR ", i_flistp);
	...
	if(serv_pdp != NULL)
	{
		...
		else
		{
			...
			if(mresults_flistp)
			{
				tech = PIN_FLIST_FLD_TAKE(mresults_flistp, MSO_FLD_TECHNOLOGY, 1, ebufp);
        ...
        pin_errbuf_t    *ebufp)
{
	...
	char		*device_id = NULL;		
	char		search_template[100] = " select X from /device where F1 = V1 and F2 = V2 and F3.type = V3 ";
        ...
        pin_errbuf_t            *ebufp)
{
	...
	PIN_FLIST_FLD_SET(s_iflistp, PIN_FLD_POID, a_pdp, ebufp);
        ...
        if (PIN_ERRBUF_IS_ERR(ebufp))
        {
                ...
                PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,"Inputs May be missing ", ebufp);
	...
	if (PIN_ERRBUF_IS_ERR(ebufp))
        {
               	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_ERROR,
                    		"fm_mso_cust_bb_hw_amc_process flist", i_flistp);
               	PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,"Failed in AMC MAIN PROCESS ", ebufp);
		PIN_FLIST_FLD_SET(i_flistp, PIN_FLD_AMOUNT, famountp, ebufp);
		*r_flistpp = i_flistp;
		return;
        }
        ...
        pin_errbuf_t    *ebufp)
{
	...
	args_flistp = PIN_FLIST_ELEM_ADD(search_inflistp, PIN_FLD_ARGS, 4, ebufp);
		...
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "Error Preparing Input for Search", ebufp);
		PIN_FLIST_DESTROY_EX(&search_inflistp, NULL);
		return;
        ...
        pin_errbuf_t    *ebufp)
{
	 ...
	 ****************************************/
        ...
        if  (PIN_ERRBUF_IS_ERR(ebufp))
        {
                ...
                PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "ERROR getting Initial Data",  ebufp);
        ...
        pin_errbuf_t    *ebufp)
{
	...
	while ((results_flistp =   // Loop through each AMC deal on customer and cancel 
			   ...
			   *action_flags == AMC_ON_CHANGE_PRE_POST )
		{
			...
			if  (PIN_ERRBUF_IS_ERR(ebufp)){
				PIN_ERR_LOG_ERROR(PIN_ERR_LEVEL_ERROR, "Error in while proividing for disconnection credit", ebufp);
...

{
	...
	if(modep && *modep == 0)
	{
        ...
        if (PIN_ERRBUF_IS_ERR(ebufp))
        {
             //On Error
               	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_ERROR,
                        "Issue Parsing Input in real Mode", i_flistp);
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
                                                "Error parsing passed input", ebufp);
               return;
		...
		if (PIN_ERRBUF_IS_ERR(ebufp))
		{
			PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_ERROR, "CANNOT PARSE INPUT" , i_flistp);
			PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
                        			"Error parsing passed input", ebufp);
		...
		fm_mso_cust_bb_hw_amc_get_sub_plan_details(
                                       ctxp, s_iflistp, &s_rflistp, ebufp);
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                        "SEARCH SUBSCRIPTION PLAN OUTPUT", s_rflistp);
	...
	if(!subsc_type)
		...
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_ERROR,
                      		"INPUT PASSED IS INCORRECT", s_rflistp);
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
                                "Error parsing passed input", ebufp);
                *r_flistpp = i_flistp;
                return;
