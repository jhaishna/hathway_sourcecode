	...
	pin_errbuf_t		*ebufp)
{
	...
	pin_flist_t		*tmp_flistp = NULL;
        pin_flist_t             *tmp_flistp1 = NULL;//for FUP200GB
	...
	int 			comp_prod = -1;
	int				*plan_type = NULL;
        char                    *plan_name = NULL;//FOR FUP200GB
...
SKIP:	
	...
	PIN_FLIST_FLD_COPY(mso_act_flistp, PIN_FLD_SERVICE_OBJ, pur_plan_flistp, PIN_FLD_SERVICE_OBJ, ebufp);
        if(pur_plan_flistp)
        {
                tmp_flistp1 = PIN_FLIST_ELEM_GET(pur_plan_flistp, PIN_FLD_DEALS, 0, 1, ebufp);
                plan_name       = PIN_FLIST_FLD_GET(tmp_flistp1, PIN_FLD_NAME, 0, ebufp);
                if(!strstr(plan_name,"FUP TOPUP 200GB"))
                {
