	...
	pin_errbuf_t		*ebufp)
{
	...
	else if( service_obj && (strcmp(PIN_POID_GET_TYPE(service_obj),MSO_BB)) == 0 )
	{
		...
		telco_status = PIN_FLIST_FLD_GET(args_flistp, PIN_FLD_STATUS, 0, ebufp);
	}
...
TERMINATE:
	...
	if( service_obj && (strcmp(PIN_POID_GET_TYPE(service_obj),MSO_BB)) == 0 )
	{
		...
		if (*(int*)telco_status == MSO_PROV_STATE_DEACTIVE || *(int*)telco_status == MSO_PROV_STATE_SUSPENDED
			|| *(int*)telco_status == MSO_PROV_STATE_HOLD)
		{
			PIN_FLIST_FLD_SET(args_flistp, PIN_FLD_STATUS, telco_status, ebufp);	
		}
		else
		{
