...
ROLE:
	...
	acct_update = fm_mso_add_deal(connp, ctxp, i_flistp, &ret_flistp, ebufp);
	...
	pin_errbuf_t		*ebufp)
{
	...
	{
		PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_DEBUG, "This plan is having only one purchased product !!!");
	}
	else
	{
		r_flistp = PIN_FLIST_CREATE(ebufp);
	...
	while ((res_flistp = PIN_FLIST_ELEM_GET_NEXT(srch_o_flistp, PIN_FLD_RESULTS, &elem_id, 1, &cookie, ebufp)) != (pin_flist_t *)NULL)
	{
		...
		if( (!strcmp("/mso_sb_adn_fdp", cycle_type)) || (!strcmp("/mso_sb_pkg_fdp", cycle_type)) || (!strcmp("/mso_sb_alc_fdp", cycle_type)) )
		{
			...
			if (in_purchase_end_t && *in_purchase_end_t !=0)
			{
		...
		pin_errbuf_t    *ebufp)
{
	...
	if(ca_flistp)
	{
		...
		if(strcmp(ca_id, "NA")==0 || strcmp(ca_id, "NA1")==0)
		{
