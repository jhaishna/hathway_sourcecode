	...
	pin_errbuf_t		*ebufp)
{
    ...
    // for DVBIP BLOCK LISTED Accounts based on suspension reason
    if(routing_poid)
    {
        fm_mso_get_account_info(ctxp, (poid_t *)routing_poid, &acnt_info, ebufp);
        if(acnt_info)
        {
        ...
        if(bus_type && bus_type != 0  && (bus_type == 99481100))
        {
            search_flistp = PIN_FLIST_CREATE(ebufp);
            PIN_FLIST_FLD_COPY(i_flistp, PIN_FLD_SERVICE_OBJ, search_flistp, PIN_FLD_POID, ebufp);
            catv_info_flistp = PIN_FLIST_SUBSTR_ADD(search_flistp, MSO_FLD_CATV_INFO, ebufp);
            PIN_FLIST_FLD_SET(catv_info_flistp, PIN_FLD_REASON_ID, NULL, ebufp);
            PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_reactivate_service_ service read fields input list", search_flistp);
            PCM_OP(ctxp, PCM_OP_READ_FLDS, 0, search_flistp, &search_res_flistp, ebufp);
            PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_mso_reactivate_service_ service read fields out list", search_res_flistp);
            catv_info_flistp =  PIN_FLIST_ELEM_GET(search_res_flistp, MSO_FLD_CATV_INFO, 0, 1, ebufp );
            reason_id = *(int32 *)PIN_FLIST_FLD_GET(catv_info_flistp, PIN_FLD_REASON_ID, 1, ebufp);
            if(reason_id &&   reason_id == 4)
            {
                PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,"ACCOUNT - Service reactivate status - This customer is block listed should not allow reactivation",ebufp);
                PIN_ERRBUF_RESET(ebufp);
                *r_flistpp = PIN_FLIST_CREATE(ebufp);
                ...
                PIN_FLIST_FLD_SET(*r_flistpp, PIN_FLD_ERROR_CODE, "51438", ebufp);
                PIN_FLIST_FLD_SET(*r_flistpp, PIN_FLD_ERROR_DESCR, "ACCOUNT - Service reactivate status - This customer is block listed should not allow reactivation ", ebufp);
                return;
            }
        }
    }
