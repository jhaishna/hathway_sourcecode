    ...
    pin_errbuf_t        *ebufp)
{
    ...
    time_t	postpaid_end = 1999999999;
    int32       status = 1;
    void        *vp = NULL;
    ...
    *r_flistpp = NULL;
    ...
    /************************************************************
     * Jhaishna override values need to remove this when required
     *************************************************************/
    PIN_FLIST_FLD_SET(*r_flistpp, PIN_FLD_STATUS, &status, ebufp);
    PIN_FLIST_FLD_SET(*r_flistpp, MSO_FLD_ISREGISTERED, &status, ebufp);
    PIN_FLIST_FLD_SET(*r_flistpp, PIN_FLD_EXPIRATION_T,&postpaid_end, ebufp);
    vp = PIN_FLIST_FLD_GET(*r_flistpp, PIN_FLD_ERROR_CODE, 1, ebufp);
    if (vp)
    {
        PIN_FLIST_FLD_DROP(*r_flistpp, PIN_FLD_ERROR_CODE, ebufp);
    }
    vp = PIN_FLIST_FLD_GET(*r_flistpp, PIN_FLD_ERROR_DESCR, 1, ebufp);
    if (vp)
    {
        PIN_FLIST_FLD_DROP(*r_flistpp, PIN_FLD_ERROR_DESCR, ebufp);
    }
    vp = PIN_FLIST_FLD_GET(*r_flistpp, PIN_FLD_DESCR, 1, ebufp);
    if (vp)
    {
        PIN_FLIST_FLD_DROP(*r_flistpp, PIN_FLD_DESCR, ebufp);
    }
    /***********************************************************
     * Error?
     ***********************************************************/
