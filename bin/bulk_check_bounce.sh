...
if [ $Rstatus -eq 0 ] ; then
   echo "`date`:Testnap Connection Success for bulk_check_bounce in PIN_HOME/mso/bin/"
else
   echo "`date`:Testnap Connection Failed so job skipped for bulk_check_bounce in PIN_HOME/mso/bin/" >> /home/pin/Thirdparty/BRM12/BRM/var/cron_global/global_cron_tab.log
   exit;
fi

prog_name='BOUNCE'
logfile='/home/pin/Thirdparty/BRM12/BRM/mso/apps/mso_bulk_audit/cheque_bounce.log'
...
if [ $Rstatus -eq 0 ] ; then
   ...
   echo "`date`:Testnap Connection Failed so job skipped for bulk_check_bounce in PIN_HOME/mso/apps/mso_bulk_AR" >> /home/pin/Thirdparty/BRM12/BRM/var/cron_global/global_cron_tab.log
   exit;
fi
...
do
        ...
        then
                if [[ $result -eq $fail ]];
                then
                        echo "`date`:bulk_check_bounce Job Failed" >> /home/pin/Thirdparty/BRM12/BRM/var/cron_global/global_cron_tab.log
                else
                        echo "`date`:bulk_check_bounce Job Success" >> /home/pin/Thirdparty/BRM12/BRM/var/cron_global/global_cron_tab.log
                fi
        fi
